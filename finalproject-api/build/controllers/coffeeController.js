...
const getAllMessages = async (req, res, next) => {
    let messages = await coffee_1.Message.findAll();
...
const createMessage = async (req, res, next) => {
    ...
    if (newMessage.message) {
        let created = await coffee_1.Message.create(newMessage);
...
const updateMessage = async (req, res, next) => {
    ...
    let newMessage = req.body;
    let messageFound = await coffee_1.Message.findByPk(messageId);
    if (messageFound && messageFound.id == newMessage.id
        && newMessage.message) {
        await coffee_1.Message.update(newMessage, {
            where: { id: messageId }
        });
        res.status(200).json();
    }
...
const deleteMessage = async (req, res, next) => {
    ...
    let messageFound = await coffee_1.Message.findByPk(messageId);
    if (messageFound) {
        await coffee_1.Message.destroy({
            where: { id: messageId }
        });
        res.status(200).json();
    }
